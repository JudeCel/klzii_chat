defmodule KlziiChat.Services.Permissions.Report do
  ...
  def can_use(%{role: session_member_role, account_user: %{role: account_user_role}}, %{data: data}) do
    ...
    ((has_role(session_member_role, roles)|| has_role(account_user_role, roles)) && has_allowed_from_subscription(data, "reportingFunctions"))
    |> formate_error
  end
  def can_use(%{role: session_member_role}, %{data: data}) do
    roles =  ~w(facilitator)
    ((has_role(session_member_role, roles)) && has_allowed_from_subscription(data, "reportingFunctions"))
    |> formate_error
  end
end
