defmodule KlziiChat.Dashboard.Presence do
  alias KlziiChat.{Repo, AcountUser, AccountUserView}
  ...
  def fetch("sessionsBuilder:"<> _id, entries) do
    ...
    for {key, %{metas: metas}} <- entries, into: %{} do
      {key, %{metas: metas, accountUser: AccountUserView.render("show.json", member: account_users[String.to_integer(key)])}}
    end
  end

  defp get_account_users(entries) do
    (from au in AcountUser,
