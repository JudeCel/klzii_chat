defmodule KlziiChat.Services.SessionReportingServiceTest do
  ...
  #   {:ok, report} = SessionReportingService.create_report(session.id, facilitator.id, session_topic.id, :csv, :all, :true)
  #
  ...
  #    SessionReportingService.create_report(session.id, participant.id, session_topic.id, :csv, :all, :true))
  # end
  #
  ...
  #   {:ok, _} = SessionReportingService.create_report(session.id, facilitator.id, session_topic.id, :txt, :all, true)
  ...
  #   {:ok, report} = SessionReportingService.create_report(session.id, facilitator.id, session_topic.id, :txt, :all, true)
  #
  ...
  #    assert({:error, %{not_found: "Session Topic Report not found"}} == SessionReportingService.recreate_report(123, facilitator.id))
  #  end
  #
  ...
  #    {:ok, report} = SessionReportingService.create_session_topics_reports_record(session.id, session_topic.id, :all, true, :pdf)
  #    assert({:error, %{permissions: "Action not allowed!", code: 403}} == SessionReportingService.recreate_report(report.id, participant.id))
  #  end
end
