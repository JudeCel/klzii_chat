defmodule KlziiChat.Services.DirectMessageTest do
  ...
  test "DirectMessage - cannot create message because sending to participant", %{ participant: participant, participant2: participant2, session: session } do
    ...
    {:error, %{permissions: error_message}} = DirectMessageService.create_message(session.id, params)
    assert(error_message == "Action not allowed!")
  end
  ...
  test "DirectMessage - get last messages", %{ facilitator: facilitator, participant: participant, session: session, participant2: participant2 } do
    ...
    message_1 = Map.get(messages.recieved, to_string(participant.id))
    message_2 = Map.get(messages.recieved, to_string(participant2.id))
    assert(message_1.text == @message_text2)
    assert(message_2.text == @message_text4)

    messages_2 = DirectMessageService.get_last_messages(participant.id)
    message = Map.get(messages_2.sent, to_string(facilitator.id))
    assert(message.text == @message_text2)
  end
end
