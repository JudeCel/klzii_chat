defmodule KlziiChat.Services.MailgunServiceTest do
  use KlziiChat.{ModelCase, AccountsCase}
  alias(KlziiChat.{Repo, Invite})
  alias KlziiChat.Services.MailgunService

  @mailMessageId "2ccb73bb-f9a0-2031-df63-9d2edb0dc15b@test.klzii.com"

  describe "mailgun_webhooks" do
    setup %{accountManager_core: accountManager_core} do
      invite =
        %Invite{
          userId: Map.get(accountManager_core, :UserId),
          token: "token",
          sentAt: Timex.now,
          role: "facilitator",
          accountUserId: accountManager_core.id,
    ...
    test "dropped", %{invite: %{id: id}} do
      ...
      assert({:ok, _} = MailgunService.mailgun_webhooks(params))
      update_invite = Repo.get(Invite, id)
      assert(update_invite.emailStatus == "failed")
    end

    test "delivered",%{invite: %{id: id}} do
      params = %{"Message-Id" => "<#{@mailMessageId}>",
      ...
      assert({:ok, _} = MailgunService.mailgun_webhooks(params))
      update_invite = Repo.get(Invite, id)
      assert(update_invite.emailStatus == "sent")
    end

    test "bounced", %{invite: %{id: id}} do
      params = %{"Message-Id" => "<#{@mailMessageId}>",
  ...
  describe "#detect_event" do
    ...
    test "delivered" do
      ...
      assert({:ok, update_attrs} = MailgunService.detect_event(params))
      assert(update_attrs.emailStatus == "sent")
      assert(update_attrs.webhookEvent == "delivered")
      assert(update_attrs.webhookMessage == "delivered")
    end
