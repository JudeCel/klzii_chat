defmodule KlziiChat.Services.Reports.Types.Messages.Formats.PdfTest do
  ...
  describe "messages" do
    test "topic report", %{topic_report_data: topic_report_data} do
      {:ok,_} = Messages.Formats.Pdf.processe_data(topic_report_data)
    end

    test "session report", %{session_report_data: session_report_data} do
      {:ok, _} = Messages.Formats.Pdf.processe_data(session_report_data)
    end
  end
end
