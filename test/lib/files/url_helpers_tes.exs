  ...
  describe "youtube_id" do
    ...
    test "youtube_url_validator" do
      valid_youtube_links = %{
        ...
        assert({:ok} = UrlHelpers.youtube_url_validator(valid_youtube_links.a))
        assert({:ok} = UrlHelpers.youtube_url_validator(valid_youtube_links.b))
        assert({:ok} = UrlHelpers.youtube_url_validator(valid_youtube_links.c))
        assert({:ok} = UrlHelpers.youtube_url_validator(valid_youtube_links.d))
        assert({:ok} = UrlHelpers.youtube_url_validator(valid_youtube_links.f))
        assert({:ok} = UrlHelpers.youtube_url_validator(valid_youtube_links.g))
        assert({:ok} = UrlHelpers.youtube_url_validator(valid_youtube_links.k))

        assert({:error, _} = UrlHelpers.youtube_url_validator(invalid_youtube_links.a))
    end
  end
end
